...
In this paper, our focus is to investigate a work efficiency metric that enables
true online {\itercomp}.
We do not emphasize the search driver of the {\itercomp}.
There have been many previous works that address the search mechanism of {\itercomp}\cite{???}.
For the purpose of evaluating the effectiveness of our work efficiency metric
on guiding online {\itercomp}, we use a fixed set of 500 optimization sequences
that were generated at random.
However, we appreciate that there are better search mechanisms available
but having a better search driver would only improve the whole process of the
online {\itercomp}.
