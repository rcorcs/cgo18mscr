\section{Work Efficiency} \label{sec:work}
    ...
    versions of a program, even when we run them only once with a different input each time. Similarly to existing one-shot performance
    metrics, work efficiency is simply the amount of \textit{work done per unit of time}: 
    \[
    ...
    we measure work, and Section~\ref{subsec:relaxed} explores ways to measure it without interfering with the normal operation of the
    program.

    \subsection{Work Metric} \label{subsec:workmetric}

    To build a reasonable work metric, our approach starts by assuming that each executed instruction contributes a certain amount towards
    ...
    increase the work total based on the instructions that would be executed with the unoptimized code, regardless of the version we use.
    ...
    program. Our choice to associate work with unoptimized runtime is largely arbitrary but instinctively reasonable. Unoptimized time is
    ...
    Using the experimental setup described in Section~\ref{sec:setup}, we run the unoptimized versions of the \FIXME{7} benchmarks listed
    in \FIXME{XXX} with all 1,000 available inputs. For each run, we recorded execution time and dynamic counts for all instruction types.
    ...
    instruction type that minimizes the error between measured runtime and work.\footnote{The resulting regression model uses \FIXME{70}
    ...
    Figure~\ref{fig:work-vs-O0time} shows the correlation of the work metric ($\Delta W$) with runtime for the unoptimized version of one
    ...
    \FIXME{More benchmarks and only coefficients?}.
    ...
    In contrast to the na\"ive instrumentation where each basic block records only its own amount of work, with optimal probe placement 
    the work counter increments need to take other basic blocks into account. Figure~\ref{fig:cfg-example} shows an example CFG, where the
    instrumented blocks are marked with red. In this example, reaching probe $P_2$ implies not only executing block $B_5$ but also not
    executing $B_6$.
