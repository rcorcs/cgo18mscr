...
Our relaxation strategy starts from the probes and their weights as decided by the optimal algorithm. Removing low weight probes is easy,
but we need a way to guarantee that the error in our work estimation will not grow too big. Our conservative approach, decides on an upper
bound for the error and applies it on every recursively on every loop of the CFG. Specifically, our algorithm extracts all subgraphs
