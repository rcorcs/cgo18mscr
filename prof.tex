...
easy, but we need a way to guarantee that the error in our work estimation will not grow too big.
...
Our conservative approach, decides on an upper bound for the error and applies it recursively on every loop of the CFG. Specifically, our algorithm extracts all subgraphs
...
For a program with a set of probes $\{P_0, P_1, \ldots, P_k\}$, similar to the \WCRelaxLower relaxation,
we model the \WPRelaxLower relaxation as the following 0-1 Knapsack problem:
\begin{gather*}
\textrm{max.}\quad\sum_{i=0}^{k} f(P_i)x_i,\quad
\textrm{s.t.}\quad\sum_{i=0}^{k} \varepsilon(P_i)x_i \leq M \\
x_i\in\{0,1\}, i\in\{0,\ldots,k\}
